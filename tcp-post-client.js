...
class XMLHttpRequest {
  ...
  send(body) {
    ...
    this.headers['Content-Length'] = Buffer.byteLength(body) // 告诉 服务器请求体的字节长度
    // 设置请求头
    rows.push(...Object.keys(this.headers).map(key => `${key}: ${this.headers[key]}`))
    let request = rows.join('\r\n') + '\r\n\r\n' + body // 注意这里拼接的两个换行符，缺一个都不行，一个是最后一个header的结尾，一个是空行，用了分隔请求体
...
xhr.open('POST', 'http://127.0.0.1:3001/post')
...
xhr.send(JSON.stringify({name: 'newming', age: 11}))
